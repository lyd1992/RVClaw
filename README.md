...
| llama.cpp PlannerBackend | 已实现，调用本地 OpenAI-compatible `llama-server`；巡检任务支持 deterministic fallback | `src/rvclaw/agent/planner.py`、`deploy/k3/` |
...
- CLI 能接收自然语言巡检任务。
- 本地 `llama.cpp` Planner 能生成或修复为受控 skill-call 序列。
